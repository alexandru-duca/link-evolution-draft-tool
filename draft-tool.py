import csv
import struct
from pathlib import Path

# The precomputed XOR table bytes
xorTableBytes = [
    
]

xor_table = []
for i in range(0, len(xorTableBytes), 4):
    # Convert every 4 bytes into a 32-bit unsigned integer
    xor_value = struct.unpack('<I', bytes(xorTableBytes[i:i+4]))[0]
    xor_table.append(xor_value)

def save_signature(buffer):
    # Get the save count from the buffer and increment it by 1
    save_count = struct.unpack_from('<I', buffer, 16)[0] + 1
    struct.pack_into('<I', buffer, 16, save_count)

    # Calculate the signature and update the buffer
    signature = get_signature(buffer)
    struct.pack_into('<I', buffer, 12, signature)

def get_signature(buffer):
    # Set the signature section of the buffer to zero
    for i in range(4):
        buffer[12 + i] = 0

    # Perform the CRC32 operation as a signature calculation
    crc = 0xFFFFFFFF
    for byte in buffer:
        crc = ((crc >> 8) ^ xor_table[(crc ^ byte) & 0xFF]) & 0xFFFFFFFF

    # Return the resulting signature as a 32-bit unsigned integer
    return crc & 0xFFFFFFFF


# Generate id dictionary for ydk conversion
id_dict = {}
with open('passcode.csv', newline='') as csvfile:
    reader = csv.reader(csvfile)
    next(reader)  # skip header
    for row in csv.reader(csvfile):
        card_id, passcode = int(row[0]), int(row[1])
        id_dict[passcode] = card_id

# Aggregate ydk file data from draft folder
draft = {}
DRAFT_DIR = Path(Path.cwd(), "draft")
DRAFT_DIR.mkdir(exist_ok=True)
for path in DRAFT_DIR.iterdir():
    if path.suffix == '.ydk':
        with open(path, "r") as file:
            for line in file.readlines():
                if not line.startswith('#'):
                    passcode = int(line.strip())
                    card_id = id_dict[passcode]
                    if card_id not in draft.keys():
                        draft[card_id] = 0
                    if draft[card_id] < 3:
                        draft[card_id] += 1

offset = 24008

# Read savegame file
with open("savegame.dat", "rb") as file:
    savegame = bytearray(file.read())

# Remove all cards
for i in range(offset + 3900, offset + 14968 + 1):
    savegame[i] = 0x00

# Add drafted cards:
for card_id in draft.keys():
    match draft[card_id]:
        case 1:
            savegame[offset + card_id] = 0x09
        case 2:
            savegame[offset + card_id] = 0x0A
        case 3:
            savegame[offset + card_id] = 0x0B

# Write new CRC Checksum
save_signature(savegame)

# Save changes to savegame file
with open("savegame.dat", "wb") as file:
    file.write(savegame)
